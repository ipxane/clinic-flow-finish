...
import { Plus, Trash2, Edit2, Star, Quote, Loader2 } from "lucide-react";
...
export function TestimonialsSection() {
    ...
    const handleSave = async () => {
        if (!formData.author_name || !formData.content) {
            toast({ title: "Error", description: "Name and content are required." });
            return;
        }
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <DialogFooter className="gap-2 sm:gap-0">
                        <Button variant="outline" onClick={() => setIsDialogOpen(false)} className="px-6" disabled={isSaving}>Cancel</Button>
