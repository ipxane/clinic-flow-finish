...
import { Plus, Trash2, Pencil, CalendarOff, RefreshCw, Loader2 } from "lucide-react";
...
export function HolidaysSection({
  ...
  const handleUpdateSpecific = async () => {
    ...
    try {
      ...
      toast({ title: "Error", description: error.message || "Failed to update exception.", variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleAddRecurring = async () => {
    ...
    try {
      ...
      toast({ title: "Success", description: "Recurring holiday added successfully." });
      setIsAddRecurringDialogOpen(false);
    } catch (error: any) {
      toast({ title: "Error", description: error.message || "Failed to add recurring holiday.", variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleUpdateRecurring = async () => {
    ...
    try {
      ...
      toast({ title: "Success", description: "Recurring holiday updated successfully." });
      setIsEditRecurringDialogOpen(false);
      setEditingRecurringHoliday(null);
    } catch (error: any) {
      toast({ title: "Error", description: error.message || "Failed to update recurring holiday.", variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="space-y-8">
      ...
      <Dialog open={isAddSpecificDialogOpen} onOpenChange={setIsAddSpecificDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsAddSpecificDialogOpen(false)} disabled={isSaving}>
              Cancel
            </Button>
            <Button
              onClick={handleAddSpecific}
              disabled={specificType === "long_holiday"
                ? (!specificStartDate || !specificEndDate || specificEndDate < specificStartDate || isSaving)
                : (!specificDate || isSaving)
              }
            >
      ...
      <Dialog open={isEditSpecificDialogOpen} onOpenChange={setIsEditSpecificDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsEditSpecificDialogOpen(false)} disabled={isSaving}>
              Cancel
            </Button>
            <Button
      ...
      <Dialog open={isAddRecurringDialogOpen} onOpenChange={setIsAddRecurringDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsAddRecurringDialogOpen(false)} disabled={isSaving}>
              Cancel
            </Button>
            <Button onClick={handleAddRecurring} disabled={isSaving}>
              {isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {isSaving ? "Adding..." : "Add Recurring Holiday"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isEditRecurringDialogOpen} onOpenChange={setIsEditRecurringDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsEditRecurringDialogOpen(false)} disabled={isSaving}>
              Cancel
            </Button>
            <Button onClick={handleUpdateRecurring} disabled={isSaving}>
              {isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {isSaving ? "Saving..." : "Save Changes"}
            </Button>
          </DialogFooter>
