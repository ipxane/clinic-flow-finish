...
export function useSettings() {
  ...
  const updateClinicSettings = async (updates: Partial<ClinicSettings>) => {
    if (!clinicSettings?.id) return;

    // Logic-level validation for statistics
    if (updates.marketing_fields?.statistics && updates.marketing_fields.statistics.length > 4) {
      toast({
        title: "Validation Error",
        description: "Maximum 4 statistics allowed.",
        variant: "destructive",
      });
      throw new Error("Maximum 4 statistics allowed");
    }
    ...
    try {
      ...
      if (data) {
        setClinicSettings(data as any as ClinicSettings);
        toast({
          title: "Success",
  ...
  const uploadImage = async (file: File, path: string, bucket: string = 'marketing'): Promise<string> => {
    try {
      ...
      console.error("Error in uploadImage hook:", error);
      throw error;
    }
  };

  // Submit booking request
  const createBookingRequest = async (request: any) => {
    try {
      const { data, error } = await supabase
